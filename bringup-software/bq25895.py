...
class bq25895:
    ...
    #ILim pin enable/disable, always leave enabled
    ...
    # we don't use the resistance compensation yet
    ...
    fault_dict = dict([('Battery', battery_fault_list), ('Charge', charge_fault_list), ('Boost', boost_fault_list), ('Watchdog',watchdog_fault_list)])
    status_dict = dict([('VsysRegulation', Vsys_regulation_status_list), ('USB input', USB_input_status_list), ('power good', power_good_status_list), ('Charge', charging_status_list ), ('Vbus', Vbus_status_list)])
    #helpers
    ...
    def get_status( self ):
        ...
        status['power good'] = ( read & self.power_good_status.mask ) >> self.power_good_status.position
        status['Charge'] = ( read & self.charge_status.mask ) >> self.charge_status.position
    ...
    def get_fault( self ):
        ...
        fault['Watchdog'] = ( read & self.watchdog_fault.mask ) >> self.watchdog_fault.position
        return fault
    ...
    def init( self ):
        ...
        # Disable the boost output
        self.i2c.writeto_mem( self.ADDRESS, self.otg_boost.register, 0x1A )
        # start ADC conversions running at a 1s interval
        self.i2c.writeto_mem( self.ADDRESS, self.conversion_start.register, 0x7D )
